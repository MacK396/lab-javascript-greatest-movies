...
let dramaScore = moviesArray.filter (obj => obj.genre.includes ('Drama'))
if (dramaScore.length === 0) {
    return 0 
}
let newArray = dramaScore.map(element => element.score); 
let average = newArray.reduce(function(accumulator, currentValue) {
    if (!currentValue) {
        return accumulator + 0
    } else {
...
let name = moviesArray.map(element => element.title)
